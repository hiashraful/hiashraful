### Hi, I'm Ash

Software Engineer at [Cuadro Group](https://cuadrogroup.com)

I build custom systems for agencies and businesses.

**What I build:**
- Cross platform Apps - Android, Windows, MacOS, iOS
- Custom MCPs for AI integration
- AI Agents, Automations
- WordPress themes & plugins (WooCommerce, REST API, Gutenberg)
- CRM systems with AI integration

**Products:**
- [Lex Facchinetti](https://cuadrogroup.com/lex-facchinetti/) - Law Firm CRM - Unified legal dashboard with case management, AI search, and client tracking
- [Lyra AI](https://lyra.cuadrogroup.com/) - AI voice agents that answer calls, qualify leads, and book appointments autonomously, 24/7
- [Fiscuss](https://cuadrogroup.com/avatar-content-automation-workflow/) - Video content with AI avatars, from text to social video in minutes
- [Leadio](https://devash.pro/leadio) - Lead management CRM with Cal.com sync & Fathom AI
- [Arcflow](https://devash.pro/arcflow) - Interactive workflow canvas for agencies
- [AI2WP](https://devash.pro/ai2wp) - AI-to-WordPress page builder
- [Minimal Checkout](https://devash.pro/custom-checkout) - Stripe-style WooCommerce checkout
- [Libraria Digitale](https://cuadrogroup.com/libraie-digitale-prompt-library/) - A rich prompt library with management system

**Stack:** PHP, JavaScript, React, Python, WordPress, WooCommerce, REST APIs, Firebase

[devash.pro](https://devash.pro) | [LinkedIn](https://www.linkedin.com/in/hiashraful/) | [X](https://x.com/DevAshWp)
